...
import { PlusIcon, PencilIcon, TrashIcon, FilmIcon, ClipboardDocumentCheckIcon } from '@heroicons/react/24/outline';
  ...
  return (
    <div className="space-y-2">
      ...
      {lessons.length === 0 ? (
        ...
        <div className="space-y-2">
          {lessons.map((lesson) => (
            <div key={lesson.id} className="overflow-hidden rounded-lg bg-gray-50 dark:bg-gray-900">
              <div className="flex items-center justify-between px-4 py-2.5">
                ...
                <div className="flex items-center space-x-1">
                  {lesson.type === 'assessment' && (
                    <button
                      onClick={() => setOpenSubmissionsLessonId((current) => current === lesson.id ? null : lesson.id)}
                      className="p-1.5 text-gray-400 hover:text-indigo-600 rounded transition-colors"
                      aria-label="Corrigir entregas"
                    >
