...
export default function CertificatesList({ certificates, students, canRevoke, onRevoke, onDownload, onShowQr }: {
        ...
        {certificates.map((cert) => (
          <div key={cert.id} className="px-5 py-4 flex items-center justify-between gap-3">
            <div>
              ...
              {cert.signed_at && <p className="text-xs text-gray-500 dark:text-gray-400">Assinado em {new Date(cert.signed_at).toLocaleDateString('pt-BR')}</p>}
            ...
            <div className="flex items-center gap-2">
              ...
              {!cert.revoked_at && (
                <>
                  <button onClick={() => onShowQr(cert)} className="px-3 py-1.5 text-xs font-medium rounded-lg border border-gray-200 text-gray-700 hover:bg-gray-50 dark:border-gray-700 dark:text-gray-300 dark:hover:bg-gray-700">
                    QR
                  </button>
                  <button onClick={() => onDownload(cert)} className="px-3 py-1.5 text-xs font-medium rounded-lg border border-gray-200 text-gray-700 hover:bg-gray-50 dark:border-gray-700 dark:text-gray-300 dark:hover:bg-gray-700">
                    PDF
                  </button>
                </>
              )}
